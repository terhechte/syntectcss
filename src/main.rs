                    ...
                    let replaced: Vec<String> = selector
                        ...
                        .map(|s| {
                            ...
                            let replaced: Vec<String> =
                                s.split(".").map(|s| format!("{}{}", &p, &s)).collect();
...
fn main() {
    ...
    let matches = App::new("syntectcss")
        .version("0.0.1")
        .author("Benedikt Terhechte")
        .arg(
            Arg::with_name("tm-theme")
                .short("s")
                .value_name("TM-THEME")
                .help("The '.tmtheme' Textmate theme.")
                .required(true),
        )
        .arg(
            Arg::with_name("root-selector")
                .short("f")
                .value_name("ROOT-SELECTOR")
                .help("The root selector identifying the 'pre' element. Default: 'pre > code'.")
                .required(false),
        )
        .arg(
            Arg::with_name("ignore-background")
                .short("i")
                .takes_value(false)
                .help("Ignore background colors in tmthemes")
                .required(false),
        )
        .arg(
            Arg::with_name("prefix")
                .short("p")
                .value_name("PREFIX")
                .help("A prefix to use for all css classes. A prefix of 'abc' would transform the class '.keyword' to '.abc-keyword'")
                .required(false),
        )
        .get_matches();
